...
//    gStyle->SetOptStat(0);
   ...
   for (Long64_t jentry=0; jentry<nentries; jentry++) {
      ...
      Float_t  integrated_cross_section_value_BH  = 24.703;   //   pb
...
leg->AddEntry(histMassdilepton,"Inelastic (#tau^{+}#tau^{-}) cepgen","L")->SetTextColor(kBlue+1);
...
TLatex *t3a = new TLatex(0.45,0.80,"M_{N}<10 GeV; Q^{2}_{e,max}<10^{2} GeV^{2};  Q^{2}_{p,max}<10^{2} GeV^{2}");
                t3a->SetNDC();
                t3a->SetTextFont(42);
