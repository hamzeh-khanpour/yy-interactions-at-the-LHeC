...
def flux_yy_atye(w, Y, qmax2p2, qmax2p1, s_cms, pEbeam2, pEbeam1, pout=False):

#    yp = w * w / s_cms / ye

    yp1 = w * math.exp(Y)  / (2.0*pEbeam1)
    yp2 = w * math.exp(-Y) / (2.0*pEbeam2)

    if (yp1 <= 0.0 or yp1 >= 1.0 or yp2 <= 0.0 or yp2 >= 1.0):              # Hamzeh take care of tagged elastic
#        print('invalid yp1 value: ', yp1)
#        print('invalid yp2 value: ', yp2)
        return 0.0
...
def flux_yyinel_atye(w, Y, qmax2p2, qmax2p1, mNmax, s_cms, pEbeam2, pEbeam1, pout=False):
...
#    yp = w * w / s_cms / ye
    ...
    if (yp1 <= 0.0 or yp1 >= 1.0 or yp2 <= 0.0 or yp2 >= 1.0):
#        print('invalid yp1 value: ', yp1)
#        print('invalid yp1 value: ', yp1)
        return 0.0
...
def flux_el_yy_atW(Y, pEbeam2, pEbeam1, qmax2p2, qmax2p1):
...
#    print(' Y value: ', Y)
    ...
    fyyatw = integ.quad(flux_yy_atye, w0, sqrt_cms,
                        args=(Y, qmax2p2, qmax2p1, s_cms, pEbeam2, pEbeam1))
    return fyyatw
...
def flux_inel_yy_atW(Y, pEbeam2, pEbeam1, qmax2p2, mNmax, qmax2p1):
    # first calculate the ymin for e-side, p-side
...
#    ymin = w * w / s_cms
    ...
    fyyatw = integ.quad(flux_yyinel_atye, w0, sqrt_cms,
                        args=(Y, qmax2p2, qmax2p1, mNmax, s_cms, pEbeam2, pEbeam1),
			epsrel=1.e-2)
#			epsabs=1.e-5, epsrel=1.e-5)
    return fyyatw
