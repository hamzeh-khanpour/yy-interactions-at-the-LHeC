...
def compare_distributions(filename):
    ...
    ## Plot histograms
    ax.plot(bin_centers, hist_Yll_E, linestyle='.', linewidth=2, color='magenta', label='elastic (cepgen)')
    ax.plot(bin_centers, hist_Yll_QE, linestyle='.', linewidth=2, color='green', label='inelastic (cepgen)')


    #ax.errorbar(bin_centers, hist_Yll_E, xerr=DX, fmt='o', ecolor='r', capsize=5, capthick=2, markeredgewidth=2)
    #ax.errorbar(bin_centers, hist_Yll_QE, xerr=DX, fmt='o', ecolor='r', capsize=5, capthick=2, markeredgewidth=2)
    ...
    ax.set_ylabel(r'$d\sigma/dY_{higgsinos} \, [pb/GeV]$', fontdict=font2)
    ax.set_ylim(0, 0.001)
    ...
    ax.legend(title=title_label, loc='upper right', fontsize='small')


    # Add text annotations
    info_text_1 = r"LHeC ($E_{e}=50$ GeV; $E_{p}=7000$ GeV)"
    ax.text(0.05, 0.95, info_text_1, transform=ax.transAxes, fontsize=15, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.5))
 
    # Add text annotations
    info_text_2 = r"$Q^2_e<10^2$ GeV$^2$; $Q^2_p<10^2$ GeV$^2$; $M_N<10$ GeV"
    ax.text(0.05, 0.85, info_text_2, transform=ax.transAxes, fontsize=15, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.5))


    # Add text annotations
    info_text_3 = r"$M_{higgsinos}$ = 100 GeV"
    ax.text(0.05, 0.75, info_text_3, transform=ax.transAxes, fontsize=15, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.5))


    # Save the plot as a PDF and JPG file
    plt.savefig("Yll_Comparison_higgsinos_DIS2024.pdf")
    plt.savefig("Yll_Comparison_higgsinos_DIS2024.jpg")
