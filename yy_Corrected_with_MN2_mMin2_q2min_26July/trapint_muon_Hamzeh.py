...
def cs_muon_w_condition(wvalue):
...
def cs_muon_w_condition_muon(wvalue):
    ...
    cs = np.where(wvalue > mmuon, (4.0 * np.pi * alpha2 * hbarc2 ) / wvalue**2.0 *
               ( (1.0 + 4.0*mmuon**2.0/wvalue**2.0 - 8.0*mmuon**4.0/wvalue**4.0) * np.log((1.0+beta)/(1.0-beta)) - beta* (1.0 + 4.0*mmuon**2.0/wvalue**2.0)), 0.) * 1e9
...
plt.savefig("cs_muon_MN2_mMin2_q2min_Final.pdf")
plt.savefig("cs_muon_MN2_mMin2_q2min_Final.jpg")
