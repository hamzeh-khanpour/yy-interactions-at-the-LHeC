...
def flux_yy_atye(Y, w, qmax2e, qmax2p, s_cms, eEbeam, pEbeam, pout=False):
    ...
    flux_prod = 4.0 * pi**2.0 * Gamma_gamma * hbarc2 / M_H ** 2.0 * flux_y_dipole(yp, pmass, qmax2p) \
                * w * flux_y_pl(ye, emass, qmax2e)       # Hamzeh
...
def flux_yyinel_atye(Y, w, qmax2e, qmax2p, mNmax, s_cms, eEbeam, pEbeam, pout=False):
    ...
    # according to Eq.(A.1)
    flux_prod = 4.0 * pi**2.0 * Gamma_gamma * hbarc2 / M_H ** 2.0 * flux_y_inel(yp, minM2, qmax2p, mNmax) \
                * w * flux_y_pl(ye, emass, qmax2e)                  # Hamzeh
