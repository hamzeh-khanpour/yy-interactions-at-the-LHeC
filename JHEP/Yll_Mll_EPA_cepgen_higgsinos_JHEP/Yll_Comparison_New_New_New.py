...
integrated_cross_section_value_QE = 1.050e-04  # pb
...
def compare_distributions(filename):
    ...
    # Calculate statistical errors (Poisson errors)
    hist_Yll_E_errors = np.sqrt(hist_Yll_E) / (bin_width_correction * integrated_cross_section_value_E)
    hist_Yll_QE_errors = np.sqrt(hist_Yll_QE) / (bin_width_correction * integrated_cross_section_value_QE)
    ...
    ax.errorbar(bin_centers, hist_Yll_E, xerr=DX, yerr=hist_Yll_E_errors, fmt='.', linestyle='None', linewidth=2, color='magenta', label='elastic (cepgen)')
    ax.errorbar(bin_centers, hist_Yll_QE, xerr=DX, yerr=hist_Yll_QE_errors, fmt='.', linestyle='None', linewidth=2, color='green', label='inelastic (cepgen)')
    ...
    area_Yll_E = np.sum(hist_Yll_E) * (bins[1] - bins[0])  # Multiplying by bin width to get total area
    area_Yll_QE = np.sum(hist_Yll_QE) * (bins[1] - bins[0])  # Multiplying by bin width to get total area
