...
from wgrid_3_50_50 import *

# ROOT settings
ROOT.gStyle.SetOptStat(0)  # Remove the statistics box from the plots

# Integrated cross-section values
integrated_cross_section_value_E  = 123.93  #pb
integrated_cross_section_value_QE = 33.945  #pb
...
def compare_Mll_distributions(filename_root):
    ...
    hist_Mll_E.GetYaxis().SetTitle("d#sigma/dM_{#mu^{+}#mu^{-}} [pb/GeV]")  # Y-axis title
    hist_Mll_E.GetXaxis().SetTitle("M_{#mu^{+}#mu^{-}} [GeV] = W_{#gamma#gamma} [GeV]")  # X-axis title
    hist_Mll_E.Draw()
    ...
    latex_Mll.DrawLatex(0.15, 0.8,
                        "Q^{2}_{e,max}<10^{2} GeV^{2};  Q^{2}_{p,max}<10^{2} GeV^{2}; #color[2]{(#mu^{+}#mu^{-}) cepgen}")

    # Save the plot for Mll as a PDF file
    canvas_Mll.SaveAs("Mll_Comparison_root_mathplotlib_Final_mumu_50_50_3.pdf")
...
##################################################################
    ...
    #ax.legend(title='$M_N<10$ GeV, ($Q^2_e<10^2$ GeV$^2$; $Q^2_p<10^2$ GeV$^2$)', fontsize=20, title_fontsize=20)



    # Set labels and legend
    ax.set_xlabel("W [GeV]")
    ax.set_ylabel("$d\sigma/dW_{ep \\rightarrow e (\gamma \gamma \\to \\mu^+\\mu^-) p^{(*)}}$ [pb/GeV]")
    ax.legend(title='$M_N<3$ GeV, ($Q^2_e<50$ GeV$^2$; $Q^2_p<50$ GeV$^2$)', fontsize=20, title_fontsize=20)
...
#    ax.legend(fontsize=20)
    ...
    # Save the plot
    plt.savefig("Mll_Comparison_matplotlib_Final_mumu_50_50_3.pdf")
...
def cs_mumu_w_condition_Hamzeh(wvalue):  # Eq.62 of Physics Reports 364 (2002) 359-450
    ...
    beta = np.sqrt(np.where(1.0 - 4.0 * mmuon * mmuon / wvalue ** 2.0 >= 0.0, 1.0 - 4.0 * mmuon * mmuon / wvalue ** 2.0,
                            np.nan))
...
def trap_integ(wv, fluxv):
    ...
    for i in range(len(wv) - 2, -1, -1):
        traparea = fluxv[i] * cs_mumu_w_condition_Hamzeh(wv[i])
        wmin[i] = wv[i]
        if i == len(wv) - 2:
            integ[i] = traparea
        else:
...
compare_Mll_distributions("LHeC_Compare_mumu_3_50_50.root")
