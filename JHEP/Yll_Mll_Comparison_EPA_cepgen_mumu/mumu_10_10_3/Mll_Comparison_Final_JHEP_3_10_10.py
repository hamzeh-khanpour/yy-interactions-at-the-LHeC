...
integrated_cross_section_value_E  = 118.31  # pb
integrated_cross_section_value_QE = 32.464  # pb
...
def compare_Mll_distributions(filename_root):
    ...
    latex_Mll.DrawLatex(0.15, 0.8,
                        "Q^{2}_{e,max}<10  GeV^{2};  Q^{2}_{p,max}<10  GeV^{2}; #color[2]{(#mu^{+}#mu^{-}) cepgen}")
...
#    ax.legend(fontsize=20)
    ...
    # Save the plot
    plt.savefig("Mll_Comparison_matplotlib_Final_mumu_10_10_3.pdf")
...
compare_Mll_distributions("LHeC_Compare_mumu_3_10_10.root")
