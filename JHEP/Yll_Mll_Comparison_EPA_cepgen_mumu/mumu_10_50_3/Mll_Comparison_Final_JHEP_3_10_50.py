...
from wgrid_3_10_50 import *

# ROOT settings
ROOT.gStyle.SetOptStat(0)  # Remove the statistics box from the plots

# Integrated cross-section values
integrated_cross_section_value_E  = 1.18365977e+02  # pb
integrated_cross_section_value_QE = 3.25435017e+01  # pb
...
def compare_Mll_distributions(filename_root):
    ...
    latex_Mll.DrawLatex(0.15, 0.8,
                        "Q^{2}_{e,max}<10  GeV^{2};  Q^{2}_{p,max}<50  GeV^{2}; #color[2]{(#mu^{+}#mu^{-}) cepgen}")

    # Save the plot for Mll as a PDF file
    canvas_Mll.SaveAs("Mll_Comparison_root_mathplotlib_Final_mumu_10_10_3.pdf")
...
##################################################################
    ...
    ax.legend(title='$M_N<3$ GeV, ($Q^2_e<10$ GeV$^2$; $Q^2_p<50$ GeV$^2$)', fontsize=20, title_fontsize=20)
...
#    ax.legend(fontsize=20)
    ...
    # Save the plot
    plt.savefig("Mll_Comparison_matplotlib_Final_mumu_10_50_3.pdf")
...
compare_Mll_distributions("LHeC_Compare_mumu_3_10_50.root")
