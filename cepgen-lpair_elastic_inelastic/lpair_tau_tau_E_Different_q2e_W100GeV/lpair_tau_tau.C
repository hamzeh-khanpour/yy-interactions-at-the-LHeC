...
// **********************************************************************
    ...
    Float_t bin_width_correction = 1000.0/1000.0;
...
//      Yll = Etall -  TMath::Cos(Thetall)/2.0 * (Mtau/Ptll)* (Mtau/Ptll);
     ...
     target = new TFile ("LHeC_E_10_10.root","recreate");
     target->cd();
...
TLatex *t3a = new TLatex(0.40,0.80,"Q^{2}_{e,max}<10^{3} GeV^{2};  Q^{2}_{p,max}<10^{3} GeV^{2}; #color[4]{W > 100 GeV}");
                t3a->SetNDC();
                t3a->SetTextFont(42);
...
c8->SaveAs("q2e_LHeC_1000GeV2.pdf");
